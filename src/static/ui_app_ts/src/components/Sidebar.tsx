...
import {selectZoom} from "../store/mapUiSlice.ts";
...
const FilterDigest = () => {
  ...
  const { plots } = useAppSelector((state: RootState) => state.mapData);
  const companyCounts = plots.reduce(
    (total, plot) => (plot?.count ?? 1) + total,
    0,
  );
  ...
  return (
    <div>
<p>{zoom}</p>
      {(!!roleFilters.root.length || !!bsdTypeFilter) && (
        <>
          ...
          {roleFilters.root.length ? (
            roleFilters.root.map((role: string) => (
              ...
              >
                {bsdTypeFilter}/ {VERBOSE_ROLES[role as keyof typeof VERBOSE_ROLES]}
              </button>
            ))
          ) : (
            <button
      ...
      {!!companyCounts && <p>{companyCounts} établissements sélectionnés</p>}
    </div>
  );
};
