...
class BsdaWorkerQuantityProcessor:
    ...
    def _preprocess_bs_data(self) -> None:
        ...
        self.quantities_signed_by_worker_by_month = (
            bsda_data[
                bsda_data["worker_work_signature_date"].between(
                    *self.data_date_interval
                )
            ]
            .groupby(pd.Grouper(key="worker_work_signature_date", freq="1M"))[
...
class TransporterBordereauxGraphProcessor:
    ...
    def _preprocess_bs_data(self) -> None:
        ...
        for bs_type, df in chain(transporter_data_dfs.items(), bs_data_dfs.items()):
            ...
            if len(df) > 0:
                ...
                df_by_month = df.groupby(pd.Grouper(key="sent_at", freq="1M"))[
                    id_col
                ].nunique()
    ...
    def _check_data_empty(self) -> bool:
        if all(
            (e is None) or (len(e) == 0)
            for e in self.transported_bordereaux_stats.values()
        ):
            return True
...
class TransportedQuantitiesGraphProcessor:
    ...
    def _check_data_empty(self) -> bool:
        if all(
            (e is None) or (len(e) == 0)
            for e in self.transported_quantities_stats.values()
        ):
            return True
